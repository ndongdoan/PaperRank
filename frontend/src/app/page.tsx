...
export default function Home() {
  ...
  const [currentQuery, setCurrentQuery] = useState({ keyword: "", author: "", limit: 10, year: ""});
  ...
  const handleSearch = async (keyword: string, author: string, limit: number, year: string = "", dFactor: number = damping, isUpdate = false) => {
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(() => {
      ...
      handleSearch(currentQuery.keyword, currentQuery.author, currentQuery.limit, currentQuery.year, damping, true);
    }, 500);
  ...
  return (
    <main className="bg-[#FDFDFD] relative min-h-screen flex flex-col">
      ...
      <div className="relative flex flex-col items-center w-full flex-1">
        ...
        {/* 2. Searchbar */}
        <div className={`relative z-50 w-full transition-all duration-1000 ${hasSearched ? "pt-26" : "pt-0"}`}>
              ...
              <div className="h-full w-full bg-white rounded-[40px] border border-gray-100 shadow-2xl relative overflow-hidden">
                ...
                <div className="absolute top-6 left-6 z-20 bg-white/60 backdrop-blur-xl p-5 rounded-3xl border border-white/20 shadow-sm flex flex-col gap-4 min-w-60">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-mono font-bold text-blue-600">{damping}</span>
                  </div>
                  ...
                  <div className="pt-2 border-t border-gray-100 flex items-center justify-between text-[10px] text-gray-400 font-medium">
                    <span className="text-[10px] text-gray-400 font-medium">
                        {isRecalculating ? "Recalculating matrix..." : "Matrix synchronized"}
                    </span>
                      <RefreshCw size={12} 
                ...
                <div className="w-full h-full flex flex-col md:flex-row overflow-hidden">
                  {loading ? (
                    <div className="flex-1 text-center flex flex-col items-center justify-center">
                      ...
                      <p className="font-light italic text-gray-400">Rendering graph...</p>
                    </div>
                  ) : (
                    <>
                      {/* Graph on the left */}
                      <div className="flex-3 relative border-r border-gray-50">
                        {graphData && graphData.nodes.length > 0 ? (
                          <GraphView data={graphData} selectedId={selectedId} onSelect={setSelectedId} />
                        ) : (
                      ...
                      <div className="flex-1 bg-gray-50/50 backdrop-blur-sm overflow-y-auto p-6 flex flex-col gap-4">
                        <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                          <Network size={14} /> Ranked Papers ({graphData?.nodes.length || 0})
                        </h3>
                        ...
                        <div className="flex flex-col gap-3">
                          {[...(graphData?.nodes || [])]
                            ...
                            .map((node, index) => {
                              const authorsData = (node.authors || "Unknown") as string | string[];
                              const firstAuthor = Array.isArray(authorsData) ? authorsData[0] : authorsData?.split(',')[0];
                              const displayAuthor = firstAuthor + (Array.isArray(authorsData) && authorsData.length > 1 ? " et al." : "");
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-start gap-3">
                                    <span className={`text-[10px] font-mono font-bold w-6 h-6 flex items-center justify-center rounded-lg shrink-0 ${
                                    ...
                                    <div className="flex flex-col gap-1">
                                      ...
                                      <div className="flex items-center gap-3 text-[10px] text-gray-400 font-medium">
                                        <span className="bg-gray-100 px-1.5 py-0.5 rounded text-gray-500">Rank: {node.rank.toFixed(4)}</span>
                                        <span className="text-gray-500 font-medium">{displayAuthor}</span>
